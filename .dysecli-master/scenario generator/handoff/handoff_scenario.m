...
% Create RF scenario file without shadowing


% BS_1->Mobile link
bs1Mobile = zeros(numSeconds, 11, 'like',signalPowerReceived1_PL);
bs1Mobile(:,1) = (1:numSeconds)'* 1000; % Time in milliseconds
bs1Mobile(:,2) = 0; %numTX
bs1Mobile(:,3) = 1; %numRX
bs1Mobile(:,4) = 10*log10(signalPowerReceived1_PL).'+gainOffset; % Gain(dB)
bs1Mobile(:,5) = 0.0; % Phi (radians)
bs1Mobile(:,6) = distanceBS1.'/3e8 * 10e6; % Delay (usec)
bs1Mobile(:,7) = -v / lambda; % Doppler (Hz)
bs1Mobile(:,8) = -1000.0; % Multipath 1 Gain - Make it something small since there isn't any 
bs1Mobile(:,9) =  0.0; % Multipath 1 Delay
bs1Mobile(:,10) = -1000.0; % Multipath 2 Gain - Make it something small since there isn't any 
...
% BS_2->Mobile link
bs2Mobile = zeros(numSeconds, 11, 'like',signalPowerReceived2_PL);
bs2Mobile(:,1) = (1:numSeconds)'* 1000; % Time in milliseconds
bs2Mobile(:,2) = 0; %numTX
bs2Mobile(:,3) = 2; %numRX
bs2Mobile(:,4) = 10*log10(signalPowerReceived2_PL).'+gainOffset; % Gain(dB)
bs2Mobile(:,5) = 0.0; % Phi (radians)
bs2Mobile(:,6) = distanceBS2.'/3e8 * 10e6; % Delay (usec)
bs2Mobile(:,7) = v / lambda; % Doppler (Hz)
bs2Mobile(:,8) = -1000.0; % Multipath 1 Gain - Make it something small since there isn't any 
bs2Mobile(:,9) =  0.0; % Multipath 1 Delay
bs2Mobile(:,10) = -1000.0; % Multipath 2 Gain - Make it something small since there isn't any 
...
fileID = fopen('handoffScenario-2bs-1ms-noShad.txt','w');
...
bs1Mobile(:,7) = -v / lambda; % Doppler (Hz)
bs1Mobile(:,8) = -1000.0; % Multipath 1 Gain - Make it something small since there isn't any 
bs1Mobile(:,9) =  0.0; % Multipath 1 Gain 
bs1Mobile(:,10) = -1000.0; % Multipath 2 Gain - Make it something small since there isn't any 
bs1Mobile(:,11) = 0.0; % Multipath 2 Delay
...
bs2Mobile(:,7) = v / lambda; % Doppler (Hz)
bs2Mobile(:,8) = -1000.0; % Multipath 1 Gain - Make it something small since there isn't any 
bs2Mobile(:,9) =  0.0; % Multipath 1 Gain 
bs2Mobile(:,10) = -1000.0; % Multipath 2 Gain - Make it something small since there isn't any 
bs2Mobile(:,11) = 0.0; % Multipath 2 Delay
